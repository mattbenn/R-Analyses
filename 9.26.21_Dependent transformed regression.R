...
myData <- allData[complete.cases(allData$`Growth%.Change.(#)`),c(14,26:35)]
...
################# Multiple Regression
myData$log_change <- log(myData$`GrowthChange.(#)` + 11.5, base=10)
full.reg <- lm(log_change ~ Results.Oriented + Flexibility + Emotional.regulation + Systematic +
...
full.notransform.reg <- lm(`GrowthChange.(#)` ~ Results.Oriented + Flexibility + Emotional.regulation + Systematic +
  Competitive + Empathy + Self.Discipline + Perserverance + Openness + Leadership, data=fixedData)
summary(full.notransform.reg)
